...
int ObjectRecognition::find(Mat &_image, char GorR, int &robotCameraAngle)
{
	//-----------------------------------------------------------
	//  image = imread( "../../../New_modules1/image_014.jpeg");
	//  image = imread( "../../../TestDATA/snap-unknown-20120901-203157-1.jpeg" );
	...
	if (ret[0].Score<0.6 && ret[0].Score>0.2)
	{
		...
		checkHomography( _image, keypoints, descriptors, ret, GorR, robotCameraAngle);
...
int ObjectRecognition::find(Mat &_mat, char GorR)
{
	int d=0;
	return find(_mat, GorR, d);
}
